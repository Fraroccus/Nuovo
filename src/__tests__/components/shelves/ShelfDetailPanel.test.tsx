...
describe("ShelfDetailPanel", () => {
  ...
  function setupFetch(items: any[] = []) {
    ...
    const mock = jest.spyOn(global, "fetch" as any).mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.startsWith("/api/shelves/s1") && method === "GET") {
        return {
          ok: true,
          json: async () => ({ id: db.shelf.id, name: db.shelf.name, items: db.items }),
        } as any;
      }
      ...
      if (url.startsWith("/api/items/") && method === "PATCH") {
        ...
        if (!item) return { ok: false, json: async () => ({ error: "Not found" }), status: 404 } as any;
        if (body.delta !== undefined) {
          const next = item.quantity + body.delta;
          if (next < 0) return { ok: false, json: async () => ({ error: "Quantity cannot be negative" }), status: 400 } as any;
          item.quantity = next;
        }
  ...
  it("loads shelf items and renders list", async () => {
    const items = [
      { id: "i1", name: "Widget", sku: "W1", description: null, quantity: 2, price: 0, category: "General", shelfId: "s1" },
    ];
    setupFetch(items);
  ...
  it("adds a new item with optimistic update and refetch", async () => {
    ...
    // fill form
    fireEvent.change(screen.getByLabelText("Name"), { target: { value: "Gadget" } });
    ...
    await waitFor(() => expect(screen.getByTestId("status-success")).toHaveTextContent("Item added"));

    // ensure subsequent refetch occurred (GET called again for items)
    await waitFor(() => {
      const calls = mock.mock.calls.filter(([url, init]) => typeof url === "string" && (url as string).startsWith("/api/items?shelfId="));
  ...
  it("adjusts quantities optimistically and rolls back on failure", async () => {
    const { mock } = setupFetch([
      { id: "i1", name: "Widget", sku: "W1", description: null, quantity: 0, price: 0, category: "General", shelfId: "s1" },
    ]);
    ...
    await waitFor(() => expect(screen.getByTestId("status-error")).toHaveTextContent("Quantity cannot be negative"));
    // rolled back to 0
    expect(screen.getByText("0")).toBeInTheDocument();

    // ensure PATCH was called
    expect(mock.mock.calls.some(([url, init]) => (init as any)?.method === "PATCH")).toBe(true);
  });
  ...
  it("removes item optimistically", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("status-success")).toHaveTextContent("Item removed"));
  });
});
